% the bootstrap version induces an offset in e2e and o2o curves during the
% early bins. I still have no clear thoughts about why this happens. For
% the moment, for figure 6, I remove the bootstrapping part.
%
% Dec 28, 2022
...
for in = 1:nneurons
    ...
    twist_exe{in,1} = bintime(FR_twist{in,1},nbins);
    shift_exe{in,1} = bintime(FR_shift{in,1},nbins);
    lift_exe{in,1} = bintime(FR_lift{in,1},nbins);
    
    
    % create obs sets (for test)
    twist_obs{in,1} = bintime(FR_twist{in,2},nbins);
    shift_obs{in,1} = bintime(FR_shift{in,2},nbins);
    lift_obs{in,1} = bintime(FR_lift{in,2},nbins);
...
% save results
save('xx_108_4_single_e2o_perf_withoutBootstrap.mat', 'verif','test');
...
for irep = 1:nrep
    ...
    sampletrials1 = datasample(1:size(twist_exe),ntrial,'rep',replace);
    sampletrials2 = datasample(1:size(shift_exe),ntrial,'rep',replace);
    sampletrials3 = datasample(1:size(lift_exe),ntrial,'rep',replace);
    ...
    % in observation task
    % randomly sample 'ntrial' unique trials from each action type
    sampletrials1 = datasample(1:size(twist_obs),ntrial,'rep',replace);
    sampletrials2 = datasample(1:size(shift_obs),ntrial,'rep',replace);
    sampletrials3 = datasample(1:size(lift_obs),ntrial,'rep',replace); 
