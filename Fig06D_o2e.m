...
for in = 1:nneurons
    ...
    twist_exe{in,1} = bintime(FR_twist{row,2},nbins);
    shift_exe{in,1} = bintime(FR_shift{row,2},nbins);
    lift_exe{in,1} = bintime(FR_lift{row,2},nbins);
    
    
    % create obs sets (for test)
    twist_obs{in,1} = bintime(FR_twist{row,1},nbins);
    shift_obs{in,1} = bintime(FR_shift{row,1},nbins);
    lift_obs{in,1} = bintime(FR_lift{row,1},nbins);
...
save('for_fig06D_obs2exe.mat', 'perf');
