...
load(['E:\Projects\MN\Similarity\codes\Before_25Feb2021\S001_M',monkey,'_info'],...
    'minfoID', 'mevent', 'mcnd')
infoID = minfoID;
event = mevent;
cnd = mcnd;
...
load(['E:\Projects\MN\Similarity\codes\Before_25Feb2021\S002_M',monkey,'_neuron'],...
    'mneuron', 'mneuronID', 'mind_valid_neurons')
...
plot_kon(infoID,event,cnd, neuron,neuronID,ind_valid_trials, ID);
...
load(['E:\Projects\MN\Similarity\codes\Before_25Feb2021\S002_M',monkey,'_neuron'],...
    'vneuron', 'vneuronID', 'vind_valid_neurons')
...
function plot_kon(infoID,event,cnd, neuron,neuronID,ind_valid_trials,ID)
...
% %
% figure
% histogram([t.tch]-[t.LED])
% figure
% histogram([t.rew]-[t.tch])
...
figure
raster_many(FR_b,cmap,3)
add_events(t_b,T_before)

set(gca,'xtick',w,'xticklabel',(w-T_before).*2)
pbaspect([1 .7 1])
cleanplot
...
%     y = y+offset;
%     for ifield = [1 2 4 5]
%         x = t{iblock}(:,ifield)-t{iblock}(:,3)+T_before;
% %         rnd_vec = randperm(length(y), round(.2*length(y))); % select randomly half
%         rnd_vec = round(linspace(1,length(y),.1*length(y))); % select parts with large distance
%         y_sub = y(rnd_vec);
%         x_sub = x(rnd_vec);
%         plot(x_sub,y_sub,'color','k','linestyle','none',...
%             'marker',mark_vector{ifield},'markersize',10)
%     end
%     offset = offset+size(t{iblock},1);
% end
% end
...
for ifield = [1 2 4 5]
    ...
    rnd_vec = round(linspace(1,ntrials,.5*ntrials)); % select parts with large distance
    y_sub = y(rnd_vec);
    x_sub = x(rnd_vec);
    plot(x_sub,y_sub,'color','k','linestyle','none',...
        'marker',mark_vector{ifield},'markersize',10)
